...
% (Leave) 1. Title 
% (Good) 2. The GME section - I would reduce to the minimum. only essential 
% info that those who are not familiar can understand 
% (Good) 3. footnote. My usual style is to minimize ...
% (Good) 4. Intuition: need to explain why we did it and why it works. Simply 
% stated: we don't know the priors. If we do, we don't need that method. So 
% whenever we work with data that we have no clue of the priors, we can use 
% our approach which actually covers most possible priors (it covers a wide 
% range of priors). Then, in line with Info theory we choose the solution that 
% minimizes info distance between solution and priors. that explains why the 
% objective value serves as our best statistic (and highly correlated with the MSE)..... 
...
% current methods 
%
% Other specific from text: 
% (Good) Abstract. need to fix (e.g., last sentence) 
% (Good) Section 2. Too long - reduce. Maybe in Appendix??? not sure 
% (Leave) Section 3 first paragraph - need to refine 
% (Good) Section 5. Statements like improvement decreases with sample size... be more 
% careful. Of course it does. But this is expected for all estimators (as sample size 
% increases, estimates must improve...) 
% Tables - Figures (important): Every table/figure needs a text explanation 
% under/above that table. In quite detail so reader can understand. All needs to 
% be defined. Similarly, every Table/Figure that we refer to in the text needs to 
% be explained as well in the text, including definitions of the quantities shown etc. 
% (TO DO) Add results from new experiments
